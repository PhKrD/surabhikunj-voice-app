    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-slate-700">Security audit log</h3>
      ...
      {events.length === 0 ? (
        <div className="text-center py-10 px-6">
          ...
          <p className="text-xs text-slate-400 mt-1">Security actions will be logged here.</p>
        </div>
      ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-medium text-slate-800 text-sm font-mono">{event.action}</p>
                      {event.target && <span className="text-xs text-slate-500">· {event.target}</span>}
                    </div>
                    {event.metadata && (
                      <p className="text-xs text-slate-500 mt-0.5 font-mono">{JSON.stringify(event.metadata)}</p>
                    )}
                    <p className="text-xs text-slate-400 mt-1">{new Date(event.created_at).toLocaleString()}</p>
                  </div>
                </div>
              </CardBody>
            </Card>
